...
public class FeatureDetect2 {
    ...
    public static List<String> readExcelTests(String filename){
    ...
    public static void main(String[] args){
        ...
        File file = new File("newTest2.txt");
...
//        String path = "C:\\Users\\gyul chyoung\\Pictures\\test\\from_01.jpeg";
        ...
        List<TestParams> params = readExcelParams("C:\\Projects\\swapair\\testdata1.xlsx");

        List<String> tests = readExcelTests("C:\\Projects\\swapair\\test1.xlsx");


        int ret_num = 2; //유사 feature 개수가 몇개이상일 경우
        int dist = 20;  //기준이 되는 거리 (기준보다 가까우면 일치하는 특징)
        int corr = 0;   //예측 맞은 수
        int no_yes=0;   //다른데 같다고 한 경우
        int yes_no = 0; //같은데 다르다고 한 경우

        int curr_max = 0;   //가장많이 매칭된 수
        String predict = new String();  //예측된 이미지
        for (TestParams tp : params) {
            curr_max = 0;
            predict="";
            for (String t : tests) {
                int ret = compareFeature(path+tp.from+".jpeg", path+t+".jpeg", dist);
                if (ret > curr_max) {
                    curr_max = ret;
                    predict = t;
                }
            }
            System.out.println("from "+ tp.from + " answer "+ tp.to+ " pred "+ predict);
            if(predict.equals(tp.to)){
                corr++;
            }
